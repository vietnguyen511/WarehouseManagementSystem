...
<head>
    ...
    <title><c:choose><c:when test='${mode == "edit"}'>Edit Customer</c:when><c:otherwise>Create Customer</c:otherwise></c:choose> - Warehouse Management System</title>
    ...
    <div class="main-content-with-sidebar">
        <div class="card">
            <div class="card-header">
                <div>
                    <h1 class="card-title"><c:choose><c:when test='${mode == "edit"}'>Edit Customer</c:when><c:otherwise>Create Customer</c:otherwise></c:choose></h1>
                    <p class="card-subtitle"><c:choose><c:when test='${mode == "edit"}'>Update customer information</c:when><c:otherwise>Add a new customer</c:otherwise></c:choose></p>
                </div>
            </div>
            <div class="card-body">
                ...
                <form method="post" action="${pageContext.request.contextPath}<c:choose><c:when test='${mode == "edit"}'>/customers/edit</c:when><c:otherwise>/customers/new</c:otherwise></c:choose>" id="customerForm" novalidate>
                    <c:if test='${mode == "edit"}'>
                        <input type="hidden" name="id" value="${customer.customerId}" />
                    </c:if>
                    <div class="form-group">
                        <label class="form-label">Name <span class="text-danger">*</span></label>
                        <input class="form-input" type="text" name="name" id="name" value="${customer.name}" required />
                        <div class="field-error" id="nameError"></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Phone</label>
                        <input class="form-input" type="text" name="phone" id="phone" value="${customer.phone}" />
                        <div class="field-error" id="phoneError"></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Email</label>
                        <input class="form-input" type="email" name="email" id="email" value="${customer.email}" />
                        <div class="field-error" id="emailError"></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Address</label>
                        <input class="form-input" type="text" name="address" id="address" value="${customer.address}" />
                        <div class="field-error" id="addressError"></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Status</label>
                        <select class="form-select" name="status" id="status">
                            <c:set var="isActive" value='${customer.status}' />
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a class="btn btn-secondary" href="${pageContext.request.contextPath}/customers">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
