...
//    //Ham khoi tao phieu xuat.
//    public int createExportDetails(ExportReceipt receipt) throws SQLException {
//        String sql = "INSERT INTO ExportReceipts(customer_id, user_id, date, total_quantity, total_amount, note, created_at) VALUE (?, ?, ?, ?, ?, ?, GENERATE()";
//        try(Connection conn = dbContext.connection; PreparedStatement ps = conn.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS)) {
...
//        String sql = "SELECT * FROM ExportReiceipts";
//        try (Connection conn = dbContext.connection; PreparedStatement ps = conn.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS); ResultSet rs = ps.executeQuery()) {
//            while(rs.next()) {
